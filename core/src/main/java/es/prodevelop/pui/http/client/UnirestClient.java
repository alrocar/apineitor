...
import es.prodevelop.pui.http.client.request.HttpBodyRequest;
...
import es.prodevelop.pui.http.client.response.HttpStringResponse;
...
public class UnirestClient implements HttpClient {
	...
	public void executeAsStringAsync(final HttpRequest request,
		...
		com.mashape.unirest.request.HttpRequest uniRequest = UnirestClient
				.convertRequest(request);
		try {
			com.mashape.unirest.http.HttpResponse<String> response = uniRequest
					.asString();
	...
	public void executeAsBinaryAsync(final HttpRequest request,
		...
		com.mashape.unirest.request.HttpRequest uniRequest = UnirestClient
				.convertRequest(request);
		try {
			com.mashape.unirest.http.HttpResponse<InputStream> response = uniRequest
					.asBinary();
			UnirestClient.publishResponse(response, request, callBack, null);
		} catch (UnirestException ex) {
			UnirestClient.publishResponse(null, request, callBack, ex);
	 ...
	 * 
	 * @param response
	 *            The http response to publish
	 * @param context
	 *            The user specified context object
	 * @param completionBlock
	 *            The success and failure code block reference to invoke the
	 *            delegate
	 * @param uniException
	 *            The reported errors for getting the http response
	 * @throws APIException
	 */
	protected static void publishResponse(
			com.mashape.unirest.http.HttpResponse<?> response,
			HttpRequest request, APICallBack<HttpResponse> completionBlock,
			UnirestException uniException) throws APIException {
		...
		if (uniException == null) {
			...
			completionBlock.onFailure(context,
					new APIException(innerException.getMessage()));
		}
	}

	/**
	 * Converts a given Unirest http response into our internal http response
	 * model
	 * 
	 * @param response
	 *            The given unirest http response
	 * @return The converted http response
	 */
	public static HttpResponse convertResponse(
			com.mashape.unirest.http.HttpResponse<?> response) {
		HttpResponse httpResponse = null;

		if (response.getBody() instanceof String) {
			httpResponse = new HttpStringResponse(response.getStatus(),
					response.getHeaders().getFlatHeaders(),
					response.getRawBody(), (String) response.getBody());
		} else {
			httpResponse = new HttpResponse(response.getStatus(), response
					.getHeaders().getFlatHeaders(), response.getRawBody(),
	 ...
	 * 
	 * @param request
	 *            The given http request in internal format
	 * @return The converted unirest http request
	 */
	protected static com.mashape.unirest.request.HttpRequest convertRequest(
			HttpRequest request) {
		com.mashape.unirest.http.HttpMethod uniMethod = UnirestClient
				.convertHttpMetod(request.getHttpMethod());
		String url = request.getQueryUrl();

		// instantiate unirest request object
		com.mashape.unirest.request.HttpRequestWithBody uniRequest = new com.mashape.unirest.request.HttpRequestWithBody(
				uniMethod, url);
		...
		if (request instanceof HttpBodyRequest) {
			...
			((com.mashape.unirest.request.HttpRequestWithBody) uniRequest)
					.body(((HttpBodyRequest) request).getBody());
		} else {
			// set request fields
	...
	/**
	 * Converts a given internal http method enumeration into unirest http
	 * method unirest
	 * 
	 * @param method
	 *            The given http method enum in internal format
	 * @return The converted unirest http method enum
	 */
	protected static com.mashape.unirest.http.HttpMethod convertHttpMetod(
			HttpMethod method) {
		switch (method) {
		case POST:
	...
	public HttpRequest get(String _queryUrl, Map<String, String> _headers,
		...
		return new HttpRequest(HttpMethod.GET, _queryUrl, _headers,
				_parameters, _username, _password);
	}
	...
	public HttpRequest post(String _queryUrl, Map<String, String> _headers,
			Map<String, Object> _parameters) {
		return new HttpRequest(HttpMethod.POST, _queryUrl, _headers,
				_parameters);
	}
	...
	public HttpRequest put(String _queryUrl, Map<String, String> _headers,
			Map<String, Object> _parameters, String _username, String _password) {
		return new HttpRequest(HttpMethod.PUT, _queryUrl, _headers,
				_parameters, _username, _password);
	}
	...
	public HttpRequest patch(String _queryUrl, Map<String, String> _headers,
			Map<String, Object> _parameters) {
		return new HttpRequest(HttpMethod.PATCH, _queryUrl, _headers,
				_parameters);
	}
	...
	public HttpBodyRequest patchBody(String _queryUrl,
			Map<String, String> _headers, String _body, String _username,
			String _password) {
		return new HttpBodyRequest(HttpMethod.PATCH, _queryUrl, _headers,
				_body, _username, _password);
	}
	...
	public HttpRequest delete(String _queryUrl, Map<String, String> _headers,
			Map<String, Object> _parameters) {
		return new HttpRequest(HttpMethod.DELETE, _queryUrl, _headers,
				_parameters);
	}
	...
	public HttpBodyRequest deleteBody(String _queryUrl,
			Map<String, String> _headers, String _body) {
		return new HttpBodyRequest(HttpMethod.DELETE, _queryUrl, _headers,
				_body);
	}
